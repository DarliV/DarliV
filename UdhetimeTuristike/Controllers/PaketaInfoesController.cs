...
namespace UdhetimeTuristike.Controllers
{
    public class PaketaInfoesController : Controller
    {
        ...
        public ActionResult Index(string kerkimi)
        {
            ...
            if (User.IsInRole("Admin"))
            {
                if (!string.IsNullOrEmpty(kerkimi)) {
                    var kontakti = db.paketaInfos.Where(p => p.emriQytetit.Contains(kerkimi) || p.emriShtetit.Contains(kerkimi) || p.ApplicationUser.Email.Contains(kerkimi)).ToList();
                    return View(kontakti);
                }
                else
                {
                    var kontakti = db.paketaInfos.ToList();
                    return View(kontakti);
                }
            }
            else
            {
                if (!string.IsNullOrEmpty(kerkimi))
                {
                    var kontakti = db.paketaInfos.Where(p => (p.emriQytetit.Contains(kerkimi) || p.emriShtetit.Contains(kerkimi)) && p.ApplicationUser.Email == user).ToList();
                    return View(kontakti);
                }
                else
                {
                    var kontakti = db.paketaInfos.Where(p => p.ApplicationUser.Email.Contains(user)).ToList();
                    return View(kontakti);
                }
            }
        }
        ...
        [HttpPost]
        //[ValidateAntiForgeryToken]
        [Route("PaketaInfoes/DeleteConfirmed")]
